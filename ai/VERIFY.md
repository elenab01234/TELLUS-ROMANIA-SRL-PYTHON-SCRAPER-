...
```bash
python3 -c "from scraper.anaf import get_company_from_anaf; print(get_company_from_anaf('21147668'))"
```

Expect: `denumire == "CALLPOINT NEW EUROPE SRL"`, `cif == "21147668"`.
...
Expect: `>= 1` jobs scraped, unique URLs, titles present, CALLPOINT NEW EUROPE department filter.

## Live — peviitor SOLR

```bash
curl "https://api.peviitor.ro/v1/scraper/jobs/?cif=21147668&rows=500"
```

Expect: `success: true`. Note CIF `21147668` is shared with other peviitor
...
```bash
gh api repos/elenab01234/TELLUS-ROMANIA-SRL-PYTHON-SCRAPER-/pages --jq .html_url
curl -s -o /dev/null -w "%{http_code}\n" https://elenab01234.github.io/TELLUS-ROMANIA-SRL-PYTHON-SCRAPER-/
```

Expect: `https://elenab01234.github.io/TELLUS-ROMANIA-SRL-PYTHON-SCRAPER-/` and HTTP `200`.
...
```bash
curl -s "https://api.peviitor.ro/v1/scraper/jobs/?cif=21147668&rows=500"
```

Check `docs/jobs.md` was regenerated and jobs are visible on https://peviitor.ro (CIF `21147668`).
